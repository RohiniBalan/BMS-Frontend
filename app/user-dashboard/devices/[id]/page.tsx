...
export default function DeviceDetailsPage({
  ...
  const handleDelete = async () => {
    try {
      ...
      window.location.href = "/user-dashboard/devices";
    } catch (err: any) {
      console.error(err);
    ...
    <div className="flex flex-col gap-6">
      ...
      >
        <div>
          ...
          <p className="text-[#8899BB] mt-2 text-sm">Device ID: {device.id}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          icon={<Zap size={13} style={{ color: "#FFB300" }} />}
        />

        <StatCard
          title="Temperature"
          value={`${telemetry?.temperature ?? "--"} °C`}
          icon={<Thermometer size={13} style={{ color: "#FF5252" }} />}
        />

        <StatCard
          title="Capacity"
          value={`${device.totalCapacityKWh} kWh`}
          icon={<Gauge size={13} style={{ color: "#60a5fa" }} />}
        />
      </div>
      ...
      >
        <div className="flex justify-between mb-2">
          ...
          <span className="text-white">{telemetry?.soc ?? 0}%</span>
        </div>

        <div className="w-full bg-[#1B2438] rounded-full h-3">
          <div
            className="bg-green-500 h-3 rounded-full"
            style={{
              width: `${telemetry?.soc ?? 0}%`,
            }}
          />
        </div>
      </div>
          ...
          >
            ...
            <div className="space-y-5">
              ...
              <Info
                label="Registered At"
                value={
                  device.registration?.registeredAt
                    ? new Date(
                        device.registration.registeredAt,
                      ).toLocaleDateString()
                    : "--"
          ...
          >
            ...
            <div className="space-y-5">
              <Info
                ...
                value={device.registration?.registrationSource || "--"}
              />
