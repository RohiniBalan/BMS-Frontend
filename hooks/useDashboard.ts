...
import { getDashboardSummary, getFleetSummary, getSocDistribution, getSocTrend } from "@/services/analyticsService";
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const loginEmail =
    localStorage.getItem("bmsLoginEmail") || "";
...
const total = formattedSoc.reduce(
...
console.log("SOC TREND API RESPONSE:", st);
console.log("SOC TREND DATA:", st.data);
